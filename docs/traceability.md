...
| `domain/memory/Memory.kt::Event` | CORE-16 Event as Fundamental Fact | §31 EVENT | P06 | MVP-06, AC-14 | `architecture_invariants/EventImmutabilityTest` |
...
| `application/protocols/ExitProtocol` | CORE-13 Exitability | §51 Exit | P10 | AC-18 | `acceptance/AC18_ExitTest` |
...
Implemented; full local build including Detekt and 55 tests passes on
2026-09-25. CLI smoke and two-node SQLite/HTTPS sync are verified;
...
- [x] `test/architecture_invariants/CoreInvariantTests` — AC-15, AC-14
      (partial: hash present, signature scheme still open), AC-16,
...
- [ ] Concrete signature scheme for `Event.signature` (ADR-003a, not
      yet written) — MVP slice runs with `signature = null` (R0/R1 per
      ADR-003, acceptable for this slice's action types)
...
- [x] Full local `./gradlew build --continue --console=plain` with JDK 21:
      Detekt and all 55 tests passed; separate CI execution remains open
